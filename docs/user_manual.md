...
*   **默认比赛赛制：** 您可以为比赛预设默认的对局赛制（例如1局1胜、3局2胜、5局3胜）。
...
*   **标记获胜者与奖品发放：** 比赛进行中，您需要手动选择每组对局的获胜者，以推进比赛进程，并支持标记单方弃权或双方弃权。比赛结束后，您可以在最终排名页面为玩家发放奖品，并进行奖品信息的编辑和管理。
*   **活动组织失败处理：** 如果报名截止时未达到最少参赛人数，系统将显示"活动组织失败"。数据库状态为4种核心状态之一：正常情况下保持为"待定"状态，系统管理员可直接将数据库字段修改为"失败"，强制比赛进入"活动组织失败"状态。您可以选择编辑比赛延长报名时间。
...
*   **退出报名：** 在比赛开始前，您可以在“我的报名”页面选择退出报名。请注意，如果系统已完成对战分组，退出将被视为弃权。当玩家弃赛达到3次后，将无法报名平台上的任何比赛。
