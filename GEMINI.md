...
本项目旨在创建一个管理玩家1v1竞技比赛的平台。平台服务于两类主要用户：比赛主办方和参赛玩家。真实比赛将在主办方创建的**燕云十六声砺兵台房间**中进行。当前版本主要支持**1v1个人赛**，暂不支持5v5团队赛。系统将自动化处理部分流程，如比赛匹配和报名限制，同时赋予主办方管理比赛进程的关键权限。
...
*   **管理比赛:**
    ...
    *   **活动组织失败处理:** 如果报名截止时已报名比赛玩家未达最少参赛人数设置，系统将显示"活动组织失败"，但数据库状态保持为"待定"。主办方可以选择编辑比赛延长报名时间。
...
*   **玩家数据统计:** 统计玩家参赛次数、获得第1、2、3名的次数、弃赛次数并在个人主页中展示。
...
*   **报名与匹配:**
    ...
    *   当报名截止时间到达时，系统检查报名人数。
        *   如果报名人数达到主办方设置的“最少参赛人数”，则比赛进入“即将开始”或“比赛准备中”状态（前端显示）。对阵图将在主办方点击“开始比赛”后生成。
        *   如果报名人数未达到“最少参赛人数”，则显示“活动组织失败”（前端显示），但数据库状态保持为“待定”，主办方可通过编辑比赛延长报名时间。
*   **晋级流程:**
    *   主办方选定胜者后，系统自动将胜者匹配到下一轮比赛，直到决出最终冠军。当出现冠军后，比赛状态自动改为“已结束”。
    *   **玩家未参赛场景处理:**
        *   **一人未到场:** 默认到场玩家获胜。
        *   **两人均未到场:** 比赛结果为“双方弃权”，不产生晋级者。主办方可选择标记为“双方弃权”。
...
*   **比赛状态流转:**
    *   **待定 (pending):** 比赛创建后的初始状态，数据库实际存储状态（自动更新）。
    *   **火热报名中:** 比赛创建后，在报名截止时间之前的状态（前端显示）。
    *   **活动组织失败:** 报名时间截止后，若报名人数未达到最少要求，则前端显示此状态。数据库状态为4种核心状态之一：
        - 正常情况下保持为`pending`状态（仅前端显示变化）
        - 系统管理员可直接将数据库字段修改为`failed`，强制比赛进入"活动组织失败"状态
    *   **即将开始:** 报名截止后、比赛开始前，且报名人数达标的状态（前端显示）。
    *   **比赛准备中:** 比赛时间已到，但主办方尚未点击"开始比赛"的状态（前端显示）。
    *   **进行中 (ongoing):** 主办方点击"开始比赛"后，比赛实际开始状态（自动更新数据库）。
    *   **已结束 (finished):** 决出最终冠军后的实际结束状态（自动更新数据库）。
    *   **延期处理:** 主办方通过编辑比赛修改报名截止时间实现延期，无需额外状态。
...
*   **玩家和主播（比赛主办方）拥有公开的个人主页，用于显示个人信息和比赛相关数据，包括比赛记录和获奖记录。**
